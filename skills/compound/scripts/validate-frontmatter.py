#!/usr/bin/env python3
"""Validate .sp-compound/solutions/ frontmatter for parser-safety issues.
...
schema's required-field or enum-value rules; those are prose rules in
references/solution-schema.yaml. The intent is to prevent the
silent-data-loss bug class where YAML's quoting rules truncate or reframe
scalar values without raising.
...
def main(argv: list[str]) -> int:
    ...
    # would falsely accept `----` or `---extra` as a terminator.
    lines = text.split("\n")
    ...
    for lineno, line in enumerate(fm_text.split("\n"), start=2):
        ...
        if not val_stripped:
            # Key with no value on this line — likely a parent of a nested block
            continue
        # Already quoted or structured (block scalar, flow collection)
        if val_stripped[0] in '"\'[{|>':
            continue

        # Only ' #' (literal space-then-hash) is the YAML comment delimiter.
        # Tab-then-hash and other whitespace-then-hash are not syntactic.
        if " #" in val_stripped:
            issues.append(
        ...
        # Only ': ' (literal colon-space) triggers nested-mapping confusion.
        if ": " in val_stripped:
            issues.append(
                f"line {lineno}: '{key.strip()}' value contains ': ' — quote it. "
