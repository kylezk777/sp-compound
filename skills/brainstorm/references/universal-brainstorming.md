...
This file is loaded when brainstorm detects a non-software task (Phase 0.5). It replaces the software-specific brainstorming phases with facilitation principles for any domain. Do not follow the software brainstorming workflow. Instead, absorb these principles and facilitate the brainstorm naturally.
...
**Offer options to react to when the user is stuck.** People who can't generate from scratch can often evaluate presented options. Keep presented options to 3-5 at any decision point.
...
**Then offer next steps** using the platform's question tool when available:
- **Create a plan** -- hand off to sp-compound:plan with the decided goal and constraints
- **Save summary to disk** -- write the summary as a markdown file
